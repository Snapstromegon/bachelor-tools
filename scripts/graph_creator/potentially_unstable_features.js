  ...
  for (const browser of browsers) {
    ...
    for (const version of versions) {
      const unstableCount = await db.all(SQL`
        SELECT * FROM feature_support_notes
        JOIN feature_support ON feature_support.feature_name = feature_support_notes.feature_name AND
        feature_support.browser_version_browser_id =  feature_support_notes.browser_version_browser_id AND
        feature_support.browser_version_label =  feature_support_notes.browser_version_label

        WHERE feature_support_notes.browser_version_browser_id = ${browser.id} AND
        feature_support_notes.browser_version_label = ${version.label} AND
